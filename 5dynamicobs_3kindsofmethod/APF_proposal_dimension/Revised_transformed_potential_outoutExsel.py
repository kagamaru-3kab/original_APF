...
class obstacles():
    def __init__(self, obstacles):  
        ...
        self.dynamic_obs_id = [0,1]
        self.obs_velocity_vector = [[-1, -1],[-1,-1]]#needed proposal dimension
    ...
    def plot_obs(self, obstacles): #plot set obstacles area
        ...
        for i in range(len(obstacles)):
            if i not in obs.dynamic_obs_id:
                ...
                ellipse_angle = np.degrees(dimention.obs_direction[i])
                if dimention.denom_x_root >= dimention.denom_y_root:
                    ellipse_width = dimention.denom_x_root+ abs(obs.obs_velocity_vector[i][0])
                    ellipse_height = dimention.denom_y_root+ abs(obs.obs_velocity_vector[i][1])
                else:
                    ellipse_width = dimention.denom_y_root +  abs(obs.obs_velocity_vector[i][0])
                    ellipse_height = dimention.denom_x_root + abs(obs.obs_velocity_vector[i][1])
...
class proposal_the_other_dimension():
    ...
    """
    def __init__(self):
        self.direction_terms_forward_zero = [np.cos(np.radians(1)), np.cos(np.radians(0))]
        self.direction_terms_forward = [np.cos(np.radians(180)), np.cos(np.radians(160))]
        self.direction_terms_side = [np.cos(np.radians(160)), np.cos(np.radians(100))]
        self.direction_terms_back = [np.cos(np.radians(100)), np.cos(np.radians(1))]
        self.kx ,self.ky = 20,20 #expand cofficient ellipse
        ...
        self.dx = veh1.speed
        self.dy= veh1.speed
        self.obs_direction = [0]*len(obs.dynamic_obs_id)
        self.LP_location_1stepbefore = [0,0]
        self.LP_location_2stepbefore = [0,0]
        self.LP_location_3stepbefore = [0,0]
        self.countlimit = 100
        self.locate_dynamicobs_adddelt = copy.deepcopy(obs.obs_velocity_vector)
        self.reacting_now = 1
        self.Once_react = True 
        self.stabilizer_react_flag = 0
        self.angle_stabilizer1 = 0
        self.angle_stabilizer2 = 0
        self.angle_stabilizer3 = 0
    ...
    def update_leading_point(self,locate_goal):  #leading point moves by its potential
        ...
        while flag:
            ...
            if self.dist_leading2vehicle >= self.leading_interval+2:
                self.reset_LP_location()
                print("reset LP interval+2")
        ...
        self.LP_location_3stepbefore = self.LP_location_2stepbefore
        self.LP_location_2stepbefore = self.LP_location_1stepbefore
        self.LP_location_1stepbefore = self.locate_leading_point 
        self.stabilizer_react_flag += 1
        #print("stabilizer",self.stabilizer_react_flag)
    ...
    def judge_whereObscomefrom(self):
        ...
        if APF.dynamic_repulse_force != 0 or APF.next_LP_might_have_dynamicrepulse != 0 : # if dynamic poten effect LP
            ...
            vector4 = [self.LP_location_2stepbefore[i] - self.LP_location_3stepbefore[i] for i in range(2)]
            ...
            #print(".......vec4 LP2-LP3",vector4,self.LP_location_3stepbefore)
            #print("maxdlta",APF.next_LP_might_have_dynamicrepulse)
            ...
            magnitudeV4 = np.linalg.norm(V4)
            magnitudeV1 = np.linalg.norm(V1)
            dot_product_V2V3 = np.dot(V2, V3[0])
            dot_product_V4V3 = np.dot(V4, V3[0])
            dot_product_V4V3 = np.dot(V4, V3[0])
            dot_product_V4V1 = np.dot(V1, V4[0])
            print("maf",magnitudeV3,magnitudeV4)
            #if magnitudeV2 * magnitudeV3 != 0 and magnitudeV2:
            #   cos_theta = dot_product_V2V3 / (magnitudeV2 * magnitudeV3)
            #if magnitudeV4 * magnitudeV1 != 0 and any(vector1)!=0 :
            #    cos_theta = dot_product_V4V1 / (magnitudeV4 * magnitudeV1)
            if magnitudeV4 * magnitudeV3 != 0 and any(vector1)!=0 :
                cos_theta = dot_product_V4V3 / (magnitudeV4 * magnitudeV3)
                angle_radians = np.arccos(np.clip(cos_theta, -1.0, 1.0))
                #print("magnitude2 size",magnitudeV2)
                #print("magnitude size",magnitudeV1/magnitudeV4)
                print("angle_dot",np.degrees(angle_radians))
                #print("APFDYNAMICFORCE",APF.dynamic_repulse_force)
            ...
            #もしベクトルが０度の時，１つ前のベクトルのノルムと変化後のノルムと比べて判断
            if angle_whereobscomefrom == 1:
                angle_whereobscomefrom = -1 if magnitudeV1/magnitudeV4 < 1 else 1
            
            if magnitudeV3 < magnitudeV4:
                angle_whereobscomefrom = 2
            ...
            if self.stabilizer_react_flag > 3:
                self.Once_react = True 
    ...
    def reactiontodynamicobs(self,locate_goal): #leading point moves by vehicle position and velocity
        if self.reacting_now == 1:# need to launch this func only once
            ...
            if self.avoid_dynamicobs_flag == 1:
                ...
                decele_rate = APF.dynamic_repulse_force if APF.dynamic_repulse_force != 0 else APF.next_LP_might_have_dynamicrepulse
                APF.vehicles_speed =  veh1.veh_maxspeed*(1 - decele_rate)  #deceleretion vehicle speed upon self.total_repulsive_force
                print("decelerae",APF.vehicles_speed)
                ...
                self.dx = self.dx*(1 - decele_rate)
                self.dy = self.dy*(1 - decele_rate)
                self.first_react = True
                print("change vehicle speed because of obs coming from sideways",self.dx,self.dy, APF.vehicles_speed)
    ...
    def calc_dynamic_repulsive(self, locate_leading_point, locate_obstacles ): #calculate dynamic repulse_force  ellipse shape
        ...
        if len(obs.dynamic_obs_id) == None:
            ...
            for id in range(len(obs.dynamic_obs_id)):
                ...
                self.numer_x_offset = -locate_obstacles[obs.dynamic_obs_id[id]][0]-obs.obs_velocity_vector[id][0]
                self.numer_y_offset = -locate_obstacles[obs.dynamic_obs_id[id]][1]-obs.obs_velocity_vector[id][1]
                numer_x_origin = (locate_leading_point[0] + self.numer_x_offset)
                numer_y_origin = (locate_leading_point[1] + self.numer_y_offset)
                self.obs_direction[id] = np.arctan2(obs.obs_velocity_vector[id][1], obs.obs_velocity_vector[id][0])
                rot = np.array([[cos(self.obs_direction[id]), -sin(self.obs_direction[id])], [sin(self.obs_direction[id]), cos(self.obs_direction[id])]])
    ...
    def reset_LP_location_addvelocity(self):
        self.locate_leading_point = [veh1.locate_vehicles[0]+20*veh1.vehicle_vector[0], veh1.locate_vehicles[1]+20*veh1.vehicle_vector[1]]
        print("reset LP", self.locate_leading_point)
        self.countlimit = 200
...
def main(): 
    ...
    if APF.dist_v2goal != None:
        ...
        if judge_exist_dynamic_goal or judgereach_finalgoal:
            ...
            # Excelファイルに保存
            wb.save('4research_APF_fig\excel_date/proposal_mix_2d_ff.xlsx')
            
if __name__ == '__main__':
    fgoal = goal([70,70],10)
    obs = obstacles([[44, 44], [80, 85]])
    veh1 = vehicles([0,0])
